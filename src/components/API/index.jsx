...
import { PAGE, FOLDER_CREATED, FILE_UPLOADED, FOLDER_NAME, FILE_LIST, EDIT_VIDEO, VIDEO_LIST, FOLDER_LIST, USER_OBJ } from "../../reducer/types";
import { FolderAddOutlined, CheckCircleOutlined, ExclamationCircleOutlined, FolderOutlined } from "@ant-design/icons";
...
const getParentAssingnedRole = async (child_id) => {
  ...
  if ((tempFolders !== undefined && tempFolders !== null && tempFolders.roles === "reseller") || tempFolders.roles === "super_admin") return "self";
  if (tempFolders !== undefined && tempFolders !== null && tempFolders.access !== undefined && tempFolders.access !== null) {
    if (tempFolders.access.admin.includes(child_id)) return "admin";
    if (tempFolders.access.user.includes(child_id)) return "user";
  }
  return "viewer";
};
...
export const CreateNewFolder = async (state, dispatch, userId, folderName) => {
  ...
  const crtFolder = axios
    .post(url + "/create_folder?id=" + userId + "&foldername=" + folderName, null, {
      headers: {
        accept: "application/json",
        Authorization: "bearer " + state.token,
      },
    ...
    .then((res) => {
      if (res.data.status_code === 200) {
        dispatch({ type: "FOLDER_LIST", payload: { folderList: Array.from(new Set(state.folderList).add(folderName)) } });
...
export const deleteAfterUpload = async (uploadId) => {
  const crtFolder = await axios
    .delete(url + "/s3/multipart/" + uploadId, null, {
      headers: {
        accept: "application/json",
      },
    })
...
export const deleteFile_Folder = async (state, dispatch, userId, objectName, recursive) => {
  ...
  const getFiles = await axios
    .delete(url + "/delete_objects?id=" + userId + recflag + objectName, null, {
      headers: {
        accept: "application/json",
      },
    })
    ...
    .catch((err) => notification.open({ message: "Error in deleting file in cdn" }));
...
export const dbAddObj = async (state, dispatch, obj) => {
  const getFiles = await axios
    .post(url + `/videos`, JSON.stringify(obj), {
      headers: {
        accept: "application/json",
        Authorization: "bearer " + state.token,
      },
...
export const dbGetObjList = async (state, dispatch) => {
  const getFiles = await axios
    .get(url + `/users/${state.userId}/items/`, {
      headers: {
        accept: "application/json",
        Authorization: "bearer " + state.token,
      },
...
export const dbUpdateObj = async (state, dispatch, obj) => {
  const getFiles = await axios
    .post(url + `/users/${state.userId}/video/update/`, obj, {
      headers: {
        accept: "application/json",
        Authorization: "bearer " + state.token,
      },
...
export const dbRemoveObj = async (state, dispatch, path, recursive) => {
  const getFiles = await axios
    .post(
      url + `/users/${state.userId}/video/delete/`,
      { path: path, recursive: recursive },
      {
        headers: {
          accept: "application/json",
...
export const listPlaylist = async (state, dispatch) => {
  const getFiles = await axios
    .get(url + `/listplaylist?user_id=${state.userId}`, {
      headers: {
        accept: "application/json",
        Authorization: "bearer " + state.token,
      },
...
export const createPlaylist = async (state, dispatch, name, ftype) => {
  const getFiles = await axios
    .get(url + `/createplaylist?user_id=${state.userId}&name=${name}&ftype=${ftype}`, {
      headers: {
        accept: "application/json",
        Authorization: "bearer " + state.token,
      },
...
export const editPlaylistName = async (state, dispatch, pid, obj) => {
  
  const getFiles = await axios
    .put(
      url +
        `/editplaylist?playlistid=${pid}`, obj ,
      {
        headers: {
          accept: "application/json",
          Authorization: "bearer " + state.token,
    ...
    .then((res) => {
      notification.open({ message:  `update complete` });
    })
    .then((err) => {
      return null;
    });
...
export const deletePlaylist = async (state, dispatch, pid) => {
  
  const getFiles = await axios
    .delete(
      url +
        `/dropplaylist?playlistid=${pid}`, 
      {
        headers: {
          accept: "application/json",
          Authorization: "bearer " + state.token,
    ...
    .then((res) => {
      notification.open({ message:  `delete complete` });
    })
    .then((err) => {
      return null;
    });
...
export const updatePlaylist = async (state, dispatch, ftype, pid, vid, pos , removeflag=false) => {
  if (pos === null || pos === undefined) pos = -1;
  if (ftype === null) return;
  const getFiles = await axios
    .get(
      url +
        `/addtoplaylist?user_id=${state.userId}&playlistid=${pid}&ftype=${ftype}&videoid=${vid}&pos=${pos}&remove=${removeflag}`,
      {
        headers: {
          accept: "application/json",
          Authorization: "bearer " + state.token,
    ...
    .then((res) => {
      notification.open({ message: `${ftype} update complete` });
    })
    .then((err) => {
      return null;
    });
...
export const getPublicItems = async (state, dispatch, key) => {
  ...
  if (userId === null || userId === undefined || token === null || token === undefined) path = `/searchvideos?key=${key}`;
  const tempFolders = await axios
    .get(url + path, {
      headers: {
        accept: "application/json",
...
export const getServedLinks = async (state, dispatch, contentid, play = false) => {
  ...
  if (userId === null || userId === undefined || token === null || token === undefined) path = `/serve_links?contentid=${contentid}&play=${play}`;
...
export const convertBytes = (bytes, key) => {
  ...
  if (["KB", "MB", "GB"].includes(key) !== true) return null;
